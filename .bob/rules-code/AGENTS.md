...
### File Operations in MCP Lessons
- When MCP is available, MUST use MCP filesystem tools, NOT Node.js `fs` module
- MCP client spawns server as subprocess with stdio transport
- Check for `mcp.json` in lesson root to determine if MCP is required
...
## No Access To
- MCP tools (use Code mode for MCP-free development)
